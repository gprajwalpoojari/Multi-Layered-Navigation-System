...
std::vector<std::vector<std::pair<int, int>>> load_obstacles(std::string object_path)//, std::vector<std::vector<std::pair<int, int>>> &obstacles)
{
    ...
    int num_obs;
    try {
      num_obs = std::stoi(tempString, nullptr, 10);
    }
    catch (...){
      std::cerr << "Error with num_obs" << std::endl;
    }
     //  store the number of obstacles, which is
...
class Vgraph {
  public :
    Vgraph(int argc, char** argv) {
      ...
      // std::vector<std::vector<std::pair<int, int>>> obstacles = {{{100, 0}, {100, 50}, {150, 0}},
      ...
      //std::vector<std::pair<geometry_msgs::Point, geometry_msgs::Point>> edges;
